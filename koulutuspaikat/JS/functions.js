...
//	$('<table id="permitInfoTable"></table>').insertAfter('')
	...
	list='<li>'+ marker['Yritys'] + '</li>';
	list+='<li>' + marker['Katuosoite_orig'] + '</li>';
	list+='<li>' + 'Kieli: ' + marker['Kieli'] + '</li>';
	//list+='<li>'+ marker['luvanMyontaja'] + '</li>';
	list+='<li>' + 'Kunta: ' + marker['Kunta'] + '(' +  marker['Postinumero'] + ')' + '</li>';
...
function mapPermits(permits) {
	...
	permits.forEach(function(item,index) { 
		popUpString="Yritys: "+item['Yritys']+"<br>";
		popUpString+="Osoite: "+item['Katuosoite_orig']+"<br>";
